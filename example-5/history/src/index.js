...
async function main() {
    ...
    await messageChannel.consume(queue, async (msg) => {
        ...
        try { // Parse the message and save video path to the database.
            const parsedMsg = JSON.parse(msg.content.toString());
            console.log(`Saving videoPath: ${parsedMsg.videoPath} to the database.`);
            await historyCollection.insertOne({ 
                videoPath: parsedMsg.videoPath
            });
            console.log("Acknowledging message was handled."); // Acknowledge that the message was successfully handled.
            messageChannel.ack(msg);

        } catch (error) { // Log any errors and reject the message.
